                        ...
                        <div className="grid grid-cols-5 gap-2">
                            {alphabet.map((letter) => (
                                <button
                                    ...
                                    className={`py-1.5 rounded text-xs font-bold transition-all ${selectedLetter === letter
                                    ...
                                    <div className="p-5 flex-1 flex flex-col">
                                        ...
                                        <div className="flex-1 mb-4">
                                            {category.children && category.children.length > 0 ? (
                                                <ul className="space-y-2"> {/* Vertical Spacing */}

                                                    {/* 🔹 Sirf pehle 4 items dikhayenge */}
                                                    {category.children.slice(0, 4).map((child) => (
                                                        <li key={child._id} className="flex items-center">
                                                            <ChevronRight size={14} className="text-gray-400 mr-1 flex-shrink-0" />

                                                            <Link
                                                                to={`/category/${child.slug}`}
                                                                className="text-sm text-gray-600 hover:text-[#0C4BB2] hover:underline transition-colors line-clamp-1"
                                                                onClick={(e) => e.stopPropagation()} // 🛑 Parent click ko rokne ke liye
                                                            >
                                                                {child.name}
                                                            </Link>
                                                        </li>
                                                ...
                                                <span className="italic text-sm text-gray-400">No sub-categories</span>
                                            )}
                                        </div>

                                        {/* View Button */}
                                        {category.children.length > 4 && (
                                        <div className="flex items-center text-[#0C4BB2]  bg-blue-50 px-2 py-1 rounded-full w-[140px] text-sm font-bold group-hover:gap-2 transition-all mt-auto">
                                          + {category.children.length - 4}  View More <ChevronRight size={16} />
                                        </div>
                                          )}
                                    </div>
                                </div>
